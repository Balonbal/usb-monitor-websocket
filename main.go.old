...
func checkKey(buf []byte, pos uint, bit uint) (bool) {
	return buf[pos] & (1 << bit) == 1 << bit
}

func makeVictoria(evts chan event) func(http.ResponseWriter, *http.Request) {
...
func main() {
	...
	victoria := makeVictoria(eventChan)
	http.HandleFunc("/", victoria)
	go http.ListenAndServe(":19123", nil)
	...
	for i := 0; *num == 0 || i < *num; i++ {
		_, err := rdr.ReadContext(opCtx, buf)
		if err != nil {
			log.Fatalf("Reading from device failed: %v", err)
		}
		newState := lastState{
			Circle: checkKey(buf, 3, 5),
			Triangle: checkKey(buf, 3, 4),
			Cross: checkKey(buf, 3, 6),
			Square: checkKey(buf, 3, 7),
		}
		if newState.Circle != prev.Circle {
			evt := event{
				Button: BTN_CIRCLE,
			}
			if newState.Circle {
		...
		if newState.Square != prev.Square {
			evt := event{
				Button: BTN_SQUARE,
			}
			if newState.Square {
		...
		if newState.Triangle != prev.Triangle {
			evt := event{
				Button: BTN_TRIANGLE,
			}
